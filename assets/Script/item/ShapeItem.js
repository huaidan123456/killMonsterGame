...
cc.Class({
    ...
    collisionFunc: function () {
        ...
        for (let i = 0; i < children.length; i++) {
            let pianyiCPos = al.pAdd(cc.p(this.node.children[0].x, this.node.children[0].y), cc.p(children[i].x, children[i].y))
            let childPos = al.pAdd(this.node.position, pianyiCPos)
    ...
    checkPosFunc: function (pos) {
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            let dis = al.pDistance(cc.p(frameNode.x, frameNode.y), pos)
            if (dis <= len) {
                return frameNode
            }
        }
        ...
        for (var i = 0; i < this.GameLayer.m_maparray.length; i++) {
            ...
            if (!frameNode.isHaveFK) {
                ...
                for (var j = 1; j < children.length; j++) {
                    var len = 52 //碰撞距离
                    var childPos = al.pAdd(srcPos, cc.p(children[j].x, children[j].y))
                    //碰撞检测
                    for (var k = 0; k < this.GameLayer.m_maparray.length; k++) {
                        var tFrameNode = this.GameLayer.m_maparray[k]
                        var dis = al.pDistance(cc.p(tFrameNode.x, tFrameNode.y), childPos)
